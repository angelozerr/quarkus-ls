 ...
 *
 */
public class QuteBinaryTextDocument implements QuteTextDocument {

	private static final Logger LOGGER = Logger.getLogger(QuteBinaryTextDocument.class.getName());
	...
	public QuteBinaryTextDocument(BinaryTemplate binaryTemplate, String templateId, Map<String, String> properties,
			QuteProject project) {
		this.binaryTemplate = binaryTemplate;
		this.templateId = binaryTemplate.getPath();
	...
	private synchronized Template loadTemplate() {
		...
		try {
			TextDocument document = new TextDocument(binaryTemplate.getContent(), uri);
