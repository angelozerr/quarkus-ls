...
public class QuarkusConfigRootProvider extends AbstractPropertiesProvider {
	...
	private static final String[] ANNOTATION_NAMES = { "io.quarkus.runtime.annotations.ConfigRoot" };
	...
	private static void processConfigRoot(IJavaElement javaElement, IAnnotation configRootAnnotation,
		...
		processConfigGroup(location, extensionName, javaElement, baseKey, configPhase, javadocCache, collector,
				monitor);
	}
	...
	private static void processConfigGroup(final String location, String extensionName, IJavaElement javaElement,
			String baseKey, ConfigPhase configPhase, Map<IPackageFragmentRoot, Properties> javadocCache,
			...
			for (IJavaElement child : elements) {
				if (child.getElementType() == IJavaElement.FIELD) {
					...
					if (configGroupAnnotation != null) {
						processConfigGroup(location, extensionName, fieldClass, subKey, configPhase, javadocCache,
								collector, monitor);
					} else {
						addMetdataProperty(location, extensionName, field, fieldTypeName, fieldClass, subKey,
								defaultValue, javadocCache, configPhase, collector, monitor);
					}
				}
			}
		}
	...
	private static void addMetdataProperty(String location, String extensionName, IField field, String fieldTypeName,
			IType fieldClass, String propertyName, String defaultValue,
			Map<IPackageFragmentRoot, Properties> javadocCache, ConfigPhase configPhase, IPropertiesCollector collector,
			IProgressMonitor monitor) throws JavaModelException {
		...
		if ("boolean".equals(fieldTypeName)) {
			collector.addMetadataProperty(MicroProfileProjectInfo.DEFAULT_REFERENCE_TYPE, propertyName, type,
					ConfigItem.NO_DEFAULT.equals(defaultValue) ? "false" : defaultValue, docs, location, extensionName,
					source, enumerations, configPhase);
		} else if (isNumber(fieldTypeName)) {
			collector.addMetadataProperty(MicroProfileProjectInfo.DEFAULT_REFERENCE_TYPE, propertyName, type,
					ConfigItem.NO_DEFAULT.equals(defaultValue) ? "0" : defaultValue, docs, location, extensionName,
					source, enumerations, configPhase);
			...
			if ((rawTypeParameters[0].trim().equals("java.lang.String"))) {
				...
				processMap(field, propertyName, rawTypeParameters[1], docs, location, extensionName, source,
						configPhase, javadocCache, collector, monitor);
			}
		} else if (isList(fieldTypeName)) {
			collector.addMetadataProperty(MicroProfileProjectInfo.DEFAULT_REFERENCE_TYPE, propertyName, type,
					defaultValue, docs, location, extensionName, source, enumerations, configPhase);
		} else if (isOptional(fieldTypeName)) {
			MetadataProperty item = collector.addMetadataProperty(MicroProfileProjectInfo.DEFAULT_REFERENCE_TYPE,
					propertyName, type, defaultValue, docs, location, extensionName, source, enumerations, configPhase);
	...
	private static void processMap(IField field, String baseKey, String mapValueClass, String docs, String location,
			String extensionName, String source, ConfigPhase configPhase,
			Map<IPackageFragmentRoot, Properties> javadocCache, IPropertiesCollector collector,
			IProgressMonitor monitor) throws JavaModelException {
		...
		if ("java.util.Map".equals(mapValueClass)) {
			...
			if (type == null || isPrimitiveType(mapValueClass)) {
				...
				addMetdataProperty(location, extensionName, field, mapValueClass, null, subKey, null, javadocCache,
						configPhase, collector, monitor);
			} else {
				processConfigGroup(location, extensionName, type, subKey, configPhase, javadocCache, collector,
						monitor);
			}
		}
	}
